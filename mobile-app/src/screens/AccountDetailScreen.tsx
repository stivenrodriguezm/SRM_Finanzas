...
export default function AccountDetailScreen() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const accountTxs = txRes.data.filter((tx) => tx.account?._id === accountId);
        ...
        <View style={[styles.heroCard, { backgroundColor: accentColor }]}>
          <View style={styles.heroIconRow}>
            <View style={styles.heroIcon}>
              <Ionicons name={(account.icon || 'wallet') as any} size={28} color="#fff" />
            </View>
            {account.isLiability && (
            ...
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 24 }}>
              ...
              <View style={styles.iconGrid}>
                {ICONS.map(ic => (
                  ...
                  >
                    <Ionicons name={ic as any} size={22} color={editIcon === ic ? editColor : colors.textSecondary} />
                  </TouchableOpacity>
                ))}
              </View>
