...
export default function ReminderDetailScreen() {
  ...
  const fetchData = async () => {
    try {
      const [accRes, payRes] = await Promise.all([
        ...
        axios.get(`${API_URL}/reminders/${reminder._id}/payments`, { headers: { Authorization: `Bearer ${token}` } })
      ]);
  ...
  const submitPayment = async () => {
    ...
    setIsSubmitting(true);
    try {
      await axios.post(`${API_URL}/reminders/${reminder._id}/pay`, {
                  ...
                  >
                    <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                      <View style={[styles.accountOptionIcon, { backgroundColor: acc.color || '#E5E7EB' }]}>
                        <Ionicons name={acc.icon || 'wallet'} size={16} color="#fff" />
                      </View>
