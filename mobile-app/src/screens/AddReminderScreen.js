...
export default function AddReminderScreen() {
  ...
  const [reminderType, setReminderType] = useState(reminderToEdit?.type || 'unico'); // 'unico' o 'periodico'
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isEditing) {
        await axios.put(`${API_URL}/reminders/${reminderToEdit._id}`, payload, {
      ...
      console.log('Error saving reminder', error?.response?.data || error.message);
      Alert.alert('Error', error?.response?.data?.message || 'No se pudo guardar el recordatorio.');
    } finally {
