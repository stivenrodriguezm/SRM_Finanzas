...
type ThemeValue = 'light' | 'dark' | 'adaptive';
...
function ThemePill({ label, value, icon, currentTheme, onSelect }: ThemePillProps) {
...
interface PrivacyRowProps {
  ...
  iconColor: string;
  label: string;
  sectionKey: 'home' | 'transactions' | 'debts' | 'reminders' | 'receivables';
  value: boolean;
  onToggle: (sectionKey: PrivacyRowProps['sectionKey'], value: boolean) => void;
}

function PrivacyRow({ icon, iconBg, iconColor, label, sectionKey, value, onToggle }: PrivacyRowProps) {
  const { colors } = usePreferences();
  const styles = getStyles(colors);
...
export default function PreferencesScreen() {
  const { preferences, updatePreference, updatePrivacy, colors } = usePreferences();
  const styles = getStyles(colors);
  ...
  const handleToggleBiometric = async (value: boolean) => {
    if (value) {
      ...
      if (!available) {
        Alert.alert('No disponible', 'Tu dispositivo no tiene Face ID/Touch ID configurado.');
        return;
      }
      const success = await authenticateWithBiometrics('Confirma para activar el bloqueo de la app');
      if (!success) return;
    }
    updatePreference('biometricLockEnabled', value);
  };
      ...
      <ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.themeRow}>
            <ThemePill label="Claro" value="light" icon="sunny-outline" currentTheme={preferences.theme} onSelect={(v) => updatePreference('theme', v)} />
            <ThemePill label="Oscuro" value="dark" icon="moon-outline" currentTheme={preferences.theme} onSelect={(v) => updatePreference('theme', v)} />
            <ThemePill label="Sistema" value="adaptive" icon="phone-portrait-outline" currentTheme={preferences.theme} onSelect={(v) => updatePreference('theme', v)} />
          </View>
        </View>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.toggleRow}>
            <View style={styles.toggleInfo}>
              <Text style={styles.toggleTitle}>Bloqueo con Face ID / Touch ID</Text>
              <Text style={styles.toggleSubtitle}>Pide biometría cada vez que abres la app</Text>
            </View>
            <Switch
        ...
        <View style={styles.section}>
          ...
          <View style={styles.toggleRow}>
            <View style={styles.toggleInfo}>
              ...
              <Text style={styles.toggleSubtitle}>Recibe un aviso un día antes del vencimiento</Text>
            </View>
            <Switch
