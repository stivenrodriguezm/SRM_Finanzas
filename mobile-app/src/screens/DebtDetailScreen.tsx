...
import { Debt, Transaction, Account } from '../types/models';
...
function TransactionItem({ tx, colors }: { tx: Transaction; colors: Colors }) {
...
export default function DebtDetailScreen() {
  ...
  const fetchDebt = async () => {
    ...
    try {
      const { data } = await apiClient.get<Debt>(`/debts/${route.params.id}`);
      setDebtDetails(data);
    } catch (e) {
      console.log('Error fetching debt', getErrorMessage(e));
    } finally {
  ...
  const fetchDebtTransactions = async () => {
    ...
    setIsLoadingTx(true);
    try {
      const { data } = await apiClient.get<Transaction[]>(`/debts/${route.params.id}/transactions`);
      setTransactions(data);
    } catch (e) {
      console.log('Error fetching debt transactions', getErrorMessage(e));
    } finally {
  ...
  const handlePayment = async () => {
    ...
    try {
      await apiClient.post(`/debts/${route.params.id}/payment`, {
      ...
      console.log('Error abonando a deuda', getErrorMessage(e));
      Alert.alert('Error', getErrorMessage(e, 'No se pudo registrar el abono'));
    } finally {
  ...
  const handleConfirmIncrease = async () => {
    ...
    try {
      ...
      await apiClient.put(`/debts/${route.params.id}`, {
      ...
      console.log('Error aumentando deuda', getErrorMessage(e));
  ...
  const handleSaveIcon = async () => {
    ...
    if (route.params?.id) {
      try {
        ...
        console.log('Error saving icon', getErrorMessage(e));
      }
    }
  };
